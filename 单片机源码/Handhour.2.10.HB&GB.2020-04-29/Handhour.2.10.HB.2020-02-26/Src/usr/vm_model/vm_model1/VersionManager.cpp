...
//    printf("\n");
    ...
    case 0xA5://自定义升级通知
    {
		m_UpdateType  = manuframe.DataBuf[0];  //保存升级类型 0x00,升级MCU程序,0x01,升级FPGA程序,0x02升级安全模块系统程序
...
//								}	
		...
		//先响应再复位		
		if(m_UpdateTotalPack == nr && m_UpdateType == 0)//接收到最后一包更新标志位
		{
//			char *p= "host20181204V2.bin";
//			this->m_UpdateMcu.FLASH_Message(binlen,p,flash_Isok);
			printf("update has get sucess \n");
//			SoftReset();
		...
		else if(m_UpdateTotalPack == nr && m_UpdateType == 1)
		{
			//重启fpga
			printf("Rest Fpga\n");
		}			
