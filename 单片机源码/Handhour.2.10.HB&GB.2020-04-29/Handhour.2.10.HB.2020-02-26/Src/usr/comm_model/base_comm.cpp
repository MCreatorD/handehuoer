...
bool Base_Comm::sendFrame(CMessage *pmessage){
    ...
    printf("Sended Message name is %s,MessageID is %d\n",pmessage->m_pType->m_pName,pmessage->getMessageID());
    llrp_u64_t sn = Param_Manager::convertDeviceSN(g_pMainApplication->deviceSN);
    pmessage->setDeviceSN(sn);
 ...
 * @return
 * 接收数据帧，接收时，会自动对immediateSendMessage
 */
...
bool Base_Comm::sendMessage(CMessage *pMessage){
	return this->sendMessage(pMessage,pMessage->getMessageID());
}
...
void Base_Comm::run(){
    if(this->isReadyForRecv()==true){
				...
				printf("1. Base_Comm receive message :%8.8x\n",m_pRecvMessage);
        CMessage *pmessage = this->recvFrame();
				m_pRecvMessage = pmessage;
				printf("2. Base_Comm receive message :%8.8x\n",m_pRecvMessage);
