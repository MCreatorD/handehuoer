...
void testsenddata(unsigned short Data_I,unsigned short Data_Q,unsigned short Data_R)
{
		unsigned char counts; //记录发送buffer长度
		unsigned char i;
	
		DataScope_Get_Channel_Data(Data_I/10.0, 1 ); //将采集到的数据赋值给发送buffer
		DataScope_Get_Channel_Data(Data_Q/10.0, 2 ); //将采集到的数据赋值给发送buffer
		DataScope_Get_Channel_Data(Data_R/10.0, 3 ); //将采集到的数据赋值给发送buffer
		counts = DataScope_Data_Generate(3); 			   //获取有效buffer长度
		
		for(i=0;i<counts;++i)                              //将buffer中的数据发送到Data Scope
		{
			sendchar(DataScope_OutPut_Buffer[i]);
		}	
}
