...
//					if(((CHbReadSpec*)popspec)->getMemoryBankID()==userid){
//						return popspec;
//					}
        }
            break;
 ...
 */
CTagReportData* OperationReport_Manager::genTagReportData(vector<OpspecResult *> * pSelectResult,vector<OpspecResult *> * pAccessResults){
    ...
    if(pAccessResults!=NULL){
        ...
        for(int i=0;i<presults->size();i++){
            ...
            if(pframe==NULL){
                continue;
            }else  if((pframe->Type&0xF0)==0xB0){//如果是写分区结果返回帧（包括私有写）
                ...
                if(popspec!=NULL){
                    ...
                    {
                        //printf("Hbwirtespec result ...................\n");
                        CHbPrivateWriteSpecResult *pprivatewriteresult = new CHbPrivateWriteSpecResult();
                        pprivatewriteresult->setResult(pframe->DataBuf[0]);
                        CHbPrivateWriteSpec *pprivatewritespec = (CHbPrivateWriteSpec*)popspec;
                        llrp_u16_t opspecid = pprivatewritespec->getOpSpecID();
                        //printf("opspecid=%d\n",opspecid);
                        pprivatewriteresult->setOpSpecID(opspecid);
                        if(pframe->DataBuf[0]==2){
                            llrp_u8v_t d = llrp_u8v_t(1);
                            d.m_pValue[0] = pframe->DataBuf[1];
                            pprivatewriteresult->setResultDescription(d);
                        }
                        prepdata->addAccessSpecResult(pprivatewriteresult);
												
												CAntennaID *pantid = prepdata->getAntennaID();
												if(pantid!=NULL){
													pantid->setAntennaID(1);
												}
                        //printf("hwWriteSpec result end........................\n");
                    }
                        break;
...
CTagSelectAccessReport* OperationReport_Manager::genSelectAccessReport(int reportdata_count){
    ...
    while(m_SelecAccessResults.size()>0&&ncount>0&&repcount<reportdata_count){
        ...
        CTagReportData *prepdata = this->genTagReportData(ptagselectaccessresult->ptagSelectResult,ptagselectaccessresult->ptagAccessOpResults);
        if(prepdata!=NULL){
...
void OperationReport_Manager::run(){
    ...
    if(m_SelecAccessResults.size()<5){//10
        //先将非continue操作结果OpspecResult打包成SelectAccessResult（一次Select的结果，包含Select操作和Access操作的结果）
        while(m_pOpSpecExec->getOpResultSize()>0&&count<10){
            //printf("has no continue data need report!\n");
        ...
        //再检查continue操作结果。
        count = 0;
        while(m_pOpSpecExec->getSelectAccessResultSize()>0&&count<10){
						//printf("has continue data need report!\n");
    ...
    if(m_SelectAccessReports.size()<=10){
        //printf("m_SelectAccessReports.size()<=10\n");
        CTagSelectAccessReport *pselectaccessreport = this->genSelectAccessReport(1);
        if(pselectaccessreport!=NULL){
            //printf("gen report.....................\n");
						//CTagReportData *prep = *pselectaccessreport->beginTagReportData();
						//CParameter *p = *prep->beginAccessSpecResult();
						//printf("p=%d,type num is %d\n",p,p->m_pType->m_TypeNum);
