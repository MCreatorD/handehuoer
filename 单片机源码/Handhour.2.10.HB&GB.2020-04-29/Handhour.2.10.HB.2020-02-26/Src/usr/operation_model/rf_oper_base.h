...
public:
	...
	void setAntPower(CAntennaSpec * pAntennaSpec,CRfSpec *pRfSpec);
		
	void printCurrentSelectSpec(){
		printf("m_pCurrentSelectSpec=%d\n",m_pCurrentSelectSpec);
	}
	
//2018.02.18新加场强检测
public:
    unsigned short queryRssi(int antID,int freqID);
    int queryBestfreq(int antID); //查找最优频点

//2019.08.15 ,新加标签RSSI查询
private:
    //llrp_u64_t m_lastQueryTagRssiTime;
    //short m_lastTagRssiValue;
public:
    short queryTagRssi();
};
