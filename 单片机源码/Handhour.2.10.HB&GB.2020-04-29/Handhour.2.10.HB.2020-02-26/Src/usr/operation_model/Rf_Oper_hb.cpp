...
bool Rf_Oper_HB::setPrivateWriteAB(){
	//2018.12.04新加，要求在进行私有写时，再进入continue模式。
		...
		for(int i=0;i<8;i++){
				sendbufspi[i] = *(((unsigned char *)&AB_ConfigParam)+7-i);
		}
...
bool Rf_Oper_HB::setSelectAccessParam(Tag_SelectSpec *pTagSelectSpec,Tag_AccessSpec *pTagAccessSpec,llrp_u16_t antSpecIndex,CRfSpec *pRfSpec){
    //如果要设置的参数与当前参数一至，什么也不做，直接返回真
		//printf("pTagSelectSpec=%d\n",pTagSelectSpec);
		//printCurrentSelectSpec();
...
//    printf("do set SelectAccessParam!\n");
    ...
    needSendParam = true;
		...
		isPrivateWriteOper = false; //2018.10.30新加
...
//            printf("AccessSpec is NULL!use no continue mode!\n");
 ...
 //       printf("has access\n");
        ...
        for(list<CParameter*>::iterator it=paccesscmd->beginOpSpec();
            ...
            if(popspec->m_pType->m_TypeNum+PARAM_BASE==HbWriteSpec||popspec->m_pType->m_TypeNum+PARAM_BASE==HbPrivateWriteSpec){
                haswriteop = true;
                break;
            }
        }
...
//								
//								CMemoryBank bank;
//								bank.setMemoryBankID(preadspec->getMemoryBankID());
//								bank.setPointer(preadspec->getPointer());
//								bank.setCount(preadspec->getCount());
//								
//								if(bank.getMemoryBankID()==selectuserid){//分区已在select中读过，则不再读该分区，并将原分区读改为读全区
//									bankinfos[0].BankReadFlag = 2;
//									printf("pTagSelectSpec->hasReadRepeat = true\n");
//									pTagSelectSpec->hasReadRepeat = true;
//									break;
//								}
//								Struct_TagBank_RWInfo bankinfo = this->getBankinfoFromMemBank(&bank);
...
//					bankinfo = this->getBankinfoFromMemBank(&bank);
//					pframe = this->genSecReadSetFrame(&bankinfo,1);
				...
				case HbWriteSpec: //是写，则将第一个要写的分区配入Select中读一次
				{
					...
					break;
				case HbPrivateWriteSpec:
				{
					//是私有写第二阶段，则就执行continues操作
					//if(this->isPrivateWriteOper==true){
					//	AB_ConfigParam |= 0x02;
					//}
					pframe = this->genSecReadSetFrame(NULL,0);
					currentAccessBankCount = 0;
...
//					
//					//生成专用写分区帧
//					CHbPrivateWriteSpec *pprivatewritespec = (CHbPrivateWriteSpec *)popspec;
//					
//					STRUCT_M2SEC_FRAME *pframe = sendPrivateWriteRequest(pprivatewritespec);
//					
//					isPrivateWriteOper = true;
//					
//					if(pframe!=NULL){
...
//	CMemoryBank bank;
//	
//	bank.setMemoryBankID(spec->getMemoryBankID());
//	bank.setPointer(spec->getPointer());
//	bank.setCount(spec->getCount());
//	
//	Struct_TagBank_RWInfo bankinfo = this->getBankinfoFromMemBank(&bank);
...
STRUCT_M2SEC_FRAME* Rf_Oper_HB::writeTagBankData(llrp_u8v_t targettid,CHbWriteSpec *spec){
	...
	if(pframe!=NULL){
		...
		if(m_pM2SecComm->waitSecReadyForComm(800)<0){
            printf("Rf_Oper_New:wait write bank ack frame overtime!\n");
			return NULL;
		}
...
STRUCT_M2SEC_FRAME* Rf_Oper_HB::getPrivateWriteACK(){
	STRUCT_M2SEC_FRAME *packframe = m_pM2SecComm->getFrameFromSecure(M2SEC_SPECIALWRITEACK_FRAME);
	
	if(packframe!=NULL){
		//printf("getPrivateWriteACK:");
		//MCUToSecureComm::printfSecFrame(pframe);
		if((packframe->Type&0xF0)==0xB0){
			return packframe;
		}else{
			//printf("getPrivateWriteACK:not need ack!");
...
//2018.10.10新加私有写
STRUCT_M2SEC_FRAME* Rf_Oper_HB::sendPrivateWriteRequest(CHbPrivateWriteSpec *spec){
	
	//配置私有写前，要让安全模块进入读模式。
//	this->setDefaultSecParam();
//	Delay_ms(100);
	
	
	uint8_t *contentbuf = new uint8_t[spec->getData().m_nValue*2+1];
	int contentlen = spec->getData().m_nValue*2+1;
	
	contentbuf[0] = 1; //操作符，启动写分区
	//memcpy(contentbuf+1,(uint8_t *)spec->getData().m_pValue,spec->getData().m_nValue*2);
	for(int i=0;i<spec->getData().m_nValue;i++){
		llrp_u16_t value = spec->getData().m_pValue[i];
		contentbuf[1+i*2] = value>>8;
		contentbuf[2+i*2] = value&0x00FF;
	}
	...
	MCUToSecureComm::printfSecFrame(pframe);
	if(m_pM2SecComm->sendFrameToSecure(pframe)){
		printf("Send private write frame success!\n");
	}else{
		printf("Send private write frame fail!\n");
		m_pM2SecComm->removeFrame(pframe);
...
/**
*  给安全模块设置一默认工作模式（读user0数据）
*/
void Rf_Oper_HB::setDefaultSecParam(){
	...
	printf("Default Sec module set frame is :");
  MCUToSecureComm::printfSecFrame(pframe);
	if(m_pM2SecComm->sendFrameToSecure(pframe)){
        printf("setDefaultSecParam:send select set secure module workmode frame success!\n");
	}else{
        printf("setDefaultSecParam:send select set secure module workmode frame fail!\n");
	}
	m_pM2SecComm->removeFrame(pframe);
	pframe = NULL;

	if(m_pM2SecComm->waitSecReadyForComm()<0){
        printf("setDefaultSecParam:wait set frame overtime!\n");
		return;
	}
	...
	if(pframe==NULL){
		...
		if((pframe->Type&0xF0)==0x20){
			if(pframe->OperId==6){
				if(pframe->DataBuf[0]==0){
          printf("setDefaultSecParam:set work mode success!\n");
					m_pM2SecComm->removeFrame(pframe);
					return;
				}else{
          printf("setDefaultSecParam:set work mode fail!\n");
					m_pM2SecComm->removeFrame(pframe);
					return;
				}
			}else{
        printf("setDefaultSecParam:Readed ack is not set work mode ack!\n");
				m_pM2SecComm->removeFrame(pframe);
				return;
			}
		}else{
      printf("setDefaultSecParam:Readed sec frame is not set ack!\n");
      MCUToSecureComm::printfSecFrame(pframe);
			m_pM2SecComm->removeFrame(pframe);
			return;
		}
