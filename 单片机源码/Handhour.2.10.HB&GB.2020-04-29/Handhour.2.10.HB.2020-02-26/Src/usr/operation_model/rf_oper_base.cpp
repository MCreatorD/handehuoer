...
//    }
    ...
    case 3:
		...
		/*
		* 将上位机下发的转换为空口规定的次序
		* [0]64  | [1]128     [2]137.14  [3]174.55  [4]274.29 [5]320     [6]349.09  [7]640
		* [0]64    [1]137.14  [2]174.55  [3]320     [4]128    [5]274.29  [6]349.09  [7]640
		*/
		if(revDataRateIndex == 1) 
		{
			revDataRateIndex = 4; 
		}
		else if(revDataRateIndex == 2) 
		{
			revDataRateIndex = 1; 
		}		
		else if(revDataRateIndex == 3) 
		{
			revDataRateIndex = 2; 
		}			
		else if(revDataRateIndex == 4) 
		{
			revDataRateIndex = 5; 
		}		
		else if(revDataRateIndex == 5) 
		{
			revDataRateIndex = 3; 
		}				
...
void Rf_Oper_Base::setAntPower(CAntennaSpec * pAntennaSpec,CRfSpec *pRfSpec){
    ...
    //////////////////////////////////////////////////////
	CAntennaConfiguration *antcfg = NULL;
    if(pRfSpec->beginAntennaConfiguration()==pRfSpec->endAntennaConfiguration()){
        printf("AntennaConfiguration is null! Use default configuration!\n");
        CSetDeviceConfig *pdevicecfg = g_pMainApplication->getParamManager()->getDeviceConfig();
        //有默认配置采用默认配置，无则生成一个默认的配置
        if(pdevicecfg->beginAntennaConfiguration()!=pdevicecfg->endAntennaConfiguration()){
            ...
            //printf("nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn\n");
            antcfg = g_pMainApplication->getParamManager()->getDefalutAntennaConfiguration(1); //默认1号天线
            g_pMainApplication->getParamManager()->getDeviceConfig()->addAntennaConfiguration(antcfg);
        }
    }else
    {
	...
	if(antcfg!=NULL){
		llrp_u16_t powerindex = antcfg->getTransmitPowerIndex();
		//RFPowerON(powerindex*100+700);
	}
...
llrp_u64_t Rf_Oper_Base::getFPGAVersion(){
	...
	//FPGA_LowPower(0);//退出低功耗上电，2018.10.15新加
...
//	}
	...
	while(1)
	{
        if(this->readSPIData(receivebufspi,8+2))
		{
            printf("spi rec buf is:");
            for(int i=0;i<8+2;i++)
			{
...
/**
 * @brief Rf_Oper_Base::queryRssi
 * @param antID,1~4
 * @param freqID,0~19
 * @return
 * 获取指定天线和指定频率的函数
 */
unsigned short Rf_Oper_Base::queryRssi(int antID,int freqID){
...
//    AB_ConfigParam &= 0xAB00000000000000;
//    temp = 0x0003000000000000;

//    AB_ConfigParam |= temp;
//    //printf("queryRssi:AB_ConfigParam = %llx\n",AB_ConfigParam);
//    this->sendParamToRf(PARAM_AB,0);

    AC_ConfigParam = 0xAC00000000000000ll;
    temp = antID-1;
    AC_ConfigParam |= temp<<48;
    temp = freqID;
    AC_ConfigParam |= temp<<40;
    ...
    temp = 15; //功率固定为15，实际在RSSI检测时，并不会起作用。
    AC_ConfigParam |= temp<<32;

    //先对FPGA配置频点，天线参数，但不需要配置功率
    //任务模式
    temp = SETRFWORK_ANT;
    AC_ConfigParam |= temp<<8;
    ...
    this->sendParamToRf(PARAM_AC,SETRFWORK_ANT);
    AC_ConfigParam &= 0xFFFFFFFFFFFF0000ll;
    //Delay_ms(2000);
    ...
    temp = SETRFWORK_POWER;
    AC_ConfigParam |= temp<<8;

    //加入除帧头外的6字节异或出的CRC
    temp = this->calcCrc8(AC_ConfigParam);
    AC_ConfigParam |= temp;
    this->sendParamToRf(PARAM_AC,SETRFWORK_POWER);
    //Delay_ms(100);
   ...
   Delay_ms(500);
    //接下来查询RSSI
    llrp_u64_t param = 0xA500000000000000ll;
...
//            printf("\n");
            ...
            if(receivebufspi[0]==0xB5){
                rssi = receivebufspi[1];
                rssi <<=8;
                rssi |= receivebufspi[2];
            }else{
    ...
    if(rssi<=0){
        rssi = 19200; //如果为0,则采用一合理数值替代
    }

    return rssi;
}

int Rf_Oper_Base::queryBestfreq(int antID){

    int rssi_table[20][2]; //二维数组，每组数据表示频率号和rssi值。
    for(int i=0;i<20;i++){
        unsigned short rssi = queryRssi(antID,i);
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        {
            if (rssi_table[j][1] > rssi_table[j + 1][1])
            {
                int temp0,temp1;
                temp0 = rssi_table[j + 1][0];
                temp1 = rssi_table[j+1][1];
                rssi_table[j+1][0] = rssi_table[j][0];
                rssi_table[j + 1][1] = rssi_table[j][1];
                rssi_table[j][0] = temp0;
                rssi_table[j][1] = temp1;
            }
        }
    }
    for(int i=0;i<20;i++){
 ...
 * @brief Rf_Oper_Base::queryRssi
 * @return
 *  查询标签RSSI信息，注意，只能在读卡的间隙调用
 */
short Rf_Oper_Base::queryTagRssi(){

//    llrp_u64_t currentutc = General_Functions::getCurrentUTC();

//    if(currentutc-m_lastQueryTagRssiTime<1000000){
//        return m_lastTagRssiValue;
//    }
//    m_lastQueryTagRssiTime = currentutc;

    unsigned short rssi = 0;

    //接下来查询RSSI
    llrp_u64_t param = 0xA400000000000000ll;
...
//                printf("\n");
    ...
    if(rssi<=0){
        rssi = 10000; //如果为0,则采用一合理数值替代
    }

    //printf("1:rssi=%x\n",rssi);

    short rssi_dis = -65.0f+(10.0f*log10((float)rssi/10.0f));

    //printf("1:rssi_dis=%d\n",rssi_dis);
    //m_lastTagRssiValue =  rssi_dis;
    return rssi_dis;
}
