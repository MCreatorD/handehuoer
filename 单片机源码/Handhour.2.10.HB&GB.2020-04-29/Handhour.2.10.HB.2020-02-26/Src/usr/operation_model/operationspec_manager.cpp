...
void OperationSpec_Manager::initDefaultSelectSpec(){
	...
	paraseletespec->setSelectCurrentState(SelectCurrentState_Inactive); //当前状态，0 Disable,1 Inactive, 2 Active
	paraseletespec->setPersistence(Persistence_Noneed); //是否需要持久化 ，0不需要，1 需要.
	...
	paraantennaconfiguration->setFrequencyIndexes(chanel);   //channel 发射频率索引数组  对应频点0-19
	paraantennaconfiguration->setForDataRateIndex(0);
	paraantennaconfiguration->setRevDataRateIndex(5);      //反向链路速率索引   320    old:0
	paraantennaconfiguration->setForModulationIndex(0);
	paraantennaconfiguration->setRevDataEncodingIndex(1);    //反向链路编码方式索引 1     old:0
	...
	//printf("1.initDefaultSelectSpec is success \n");
}
...
void OperationSpec_Manager::setSelectFromAccess(Tag_SelectSpec *pSpec){
    ...
    for(list<Tag_AccessSpec*>::iterator it=m_pAccessSpecList->begin();
        ...
        //Access的selectID相符或等于0（即所有Select）
        if((paccessspec->getAccessSpec()->getSelectSpecID()==selectid||
                paccessspec->getAccessSpec()->getSelectSpecID()==0)&&paccessspec->isValid==true){
            paccessspec->retain();
...
//    Delay_ms(2000);
		...
		if(this->findValidSelectByID(specid).size()<=0){  //查找相应ID的TagSelect，返回Select的集合   size()<=0 不存在
        //printf("addSelectSpec:not find specid!\n");
				//HbFrame_Operation::printMessage(pAddSelectSpec);
				//HbFrame_Operation::echoMessage(pAddSelectSpec);
        ...
        if(ptagspec->getSelectSpec()->getPersistence()==Persistence_Need&&isNeedSave==true){//如当前配置的规则需要持久化，则保存该规则
            this->writePersisSelect(ptagspec->getSelectSpec()->getSelectSpecID(),ptagspec->getSelectSpec());
        }
...
bool OperationSpec_Manager::addAccessSpec(CAddAccessSpec *pAddAccessSpec,bool isNeedSave){
		...
		//HbFrame_Operation::echoMessage(pAddAccessSpec);
    ...
    for(list<Tag_AccessSpec*>::iterator it=m_pAccessSpecList->begin();
        ...
        if(pspec->isValid==false&&pspec->isRetainZero()){
            ...
            m_pAccessSpecList->erase(tempit);
						//printf("m_pAccessSpecList->size = %d..\n",m_pAccessSpecList->size());
        }
    }
...
void OperationSpec_Manager::writePersisSelect(llrp_u32_t selectid,CSelectSpec *pSelectSpec){
...
//        int fd = open(filename.c_str(),O_WRONLY|O_CREAT|O_TRUNC);
//        if(fd<0){
//            printf("open current access spec file fail!\n");
//        }else{
//            fchmod(fd, S_IRUSR|S_IWUSR);
//            int len = xmlstring.length();
//            int res = write(fd,xmlstring.c_str(),len);
//            if(res!=len){
//                printf("write current access spec file fail!\n");
//                close(fd);
//            }else{
