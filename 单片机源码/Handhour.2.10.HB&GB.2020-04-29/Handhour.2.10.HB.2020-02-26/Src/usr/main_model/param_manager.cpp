...
//    return p;
	...
	printf("deviceSN is :");
	for(int i=0;i<g_pMainApplication->deviceSN.m_nValue;i++){
		printf("0x%02x,",g_pMainApplication->deviceSN.m_pValue[i]);
	}
	printf("\n");
    llrp_utf8v_t devicename = llrp_utf8v_t(8);
		for(int i=0;i<8;i++){
			devicename.m_pValue[i] = g_pMainApplication->deviceSN.m_pValue[i];
		}
