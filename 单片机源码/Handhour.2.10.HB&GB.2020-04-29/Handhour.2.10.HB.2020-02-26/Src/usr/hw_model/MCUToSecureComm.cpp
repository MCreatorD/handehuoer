...
llrp_u16v_t MCUToSecureComm::getTagDataFromTagFrame(STRUCT_M2SEC_FRAME *pframe,CMemoryBank *pbank){
    ...
    if(pframe->DataBuf[0]==1||pframe->DataBuf[0]==2||pframe->DataBuf[0]==3){
        ...
        return data;
			...
			//2018.12.12倒序
			int s = (point)*2+1+predatalen;
			for(int i=0;i<count;i++){
				llrp_u16_t a = pframe->DataBuf[s];
				a = a<<8;
				a = a|pframe->DataBuf[s+1];
				data.m_pValue[i] = a;
				s = s + 2;
			}
			/////////////////////////////////////////////////////////////////////////////////////
...
STRUCT_M2SEC_FRAME* MCUToSecureComm::genSecSerialRequestFrame(){
    int contentlen = 3;
    unsigned char databuf[3] ;
    databuf[0] = 4;
    databuf[1] = 1;
...
void MCUToSecureComm::echoSecFrame(STRUCT_M2SEC_FRAME *pframe){
    int framelen = caluFrameLen(pframe);
    uint8_t buf[1024];
    putFrameToBuf(pframe,buf);

		char echobuf[1024];
    //sprintf(echobuf,"M2Sec Frame is ");
    for(int i=0;i<framelen;i++){
        sprintf(echobuf+5*i,"0x%02x,",buf[i]);
    }
    //sprintf(echobuf,"\n");
		
		while(1){
			CommUSARTx_SendData((unsigned char *)echobuf,strlen(echobuf));
			Delay_ms(1000);
		}
}
